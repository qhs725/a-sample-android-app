        ...
        for (int i = 0; i < access_group.length(); i++) {
            ...
            if (group.has("Members") && group.getJSONArray("Members").length() > 0) {
                ...
                for (int v = 0; v < groupMembers.length(); v++) {
                    ...
                    db.insertMember(member.getString("USERID"), group.getString("GROUPID"), group.getString("ORGANIZATIONID"), member.getString("NAME_FIRST") + " " + member.getString("NAME_LAST"), member.getString("ROLE_NAME"));//, member.getString("NUMBER"));
                }
            }
                ...
                for (int t = 0; t < org_groups.length(); t++) {
                    ...
                    if (group.has("Members") && group.getJSONArray("Members").length() > 0) {
                        ...
                        for (int y = 0; y < groupMembers.length(); y++) {
                            ...
                            db.insertMember(member.getString("USERID"), group.getString("GROUPID"), org.getString("ORGANIZATIONID"), member.getString("NAME_FIRST") + " " + member.getString("NAME_LAST"), member.getString("ROLE_NAME"));//, member.getString("NUMBER"));
                        }
                    }
